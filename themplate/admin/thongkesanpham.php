...
<?php if(in_array(7,$q)==false  ){
   echo '<div style="color:red; text-align:center; margin-top:55pt;"><h1>Không phận sự miễn vào !!</h1></div>';
            ...
            $idTK=$_SESSION['idTK'];
            if(isset($_GET['timein'])  && isset($_GET['timeout'])  && isset($_GET['soluongin'])){
            $donhangtungay=$_GET['timein'];
            $donhangdenngay=$_GET['timeout'];
            $soluongin=$_GET['soluongin'];
           
            $sqlll="SELECT s.tenSP, sum(c.gia1*c.soLuong1) as doanhthu 
            from sanpham s JOIN chitietdonhang c on c.idSP = s.idSP JOIN donhang d on d.idDH=c.idDH 
            WHERE d.thoiDiemGiao >= '$donhangtungay' and d.thoiDiemGiao < '$donhangdenngay' 
            GROUP by s.tenSP HAVING sum(c.gia1*c.soLuong1) ORDER BY sum(c.gia1*c.soLuong1) DESC LIMIT $soluongin;";
...
<body onload="time()">
    ...
    <div class="container-fluid al">
        ...
        <div class="card-header">
            ...
            <span>Số lượng sản phẩm cần thống kê</span>
            <input type=number name="soluongin">
            <button type="submit" name="locdonhang">Lọc</button>

        </form>
        </div>
        <?php if(isset($_GET['timein']) && isset($_GET['timeout']) && isset($_GET['soluongin'])){ ?>
